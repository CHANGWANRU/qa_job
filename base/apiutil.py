#是整个框架的心脏，它把 YAML 数据、请求发送、断言、变量提取全部串联起来
import json
import re
...
#负责将 YAML 数据中的占位符 ${...} 替换为实际的值
    def replace_load(self, data):
        ...
        if not isinstance(data, str):
            ...
            # 👉 添加：打印替换前的数据（如果是字典，会是一长串json）（小畅实验语句）
       # print("【替换前】原始数据:", str_data)
    ...
    @classmethod
    #将响应（字典）格式化为带缩进的 JSON 字符串，便于在 Allure 报告中查看
